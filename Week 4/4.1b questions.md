...
2. **(Medium) Select all that apply:** Which regular expression patterns could be used to identify dictionary entries like "ACCORD#2"?  
A. `#\d+`  
B. `\d+$`  
C. `\w+#\d+`  
D. `^[A-Z]`  

3. **(Medium)** In the context of the sentiment dictionary, what is the purpose of splitting on the "#" sign or a number at the end of a word?  
...
4. **(Hard) Select all that apply:** Which regular expression components can help you extract the root word from entries like "ACCORD#5"?  
A. Parentheses for grouping  
B. `split()` with a regex pattern  
C. Matching sequences of letters before "#"  
